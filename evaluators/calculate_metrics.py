from torchmetrics.functional import accuracy, precision, recall, auroc, f1_score, specificity
from torchmetrics import ConfusionMatrix
# note: it looks like torchmetrics uses F1Sore instread of f1, will require deeper look .
import torch
import pandas as pd
...
def calculate_metrics(pred_labels, target_labels, loss=torch.tensor([0])):
    ...
    # note item() extracts scalar val from tensor object
    epoch_metrics = {'acc': accuracy(pred_labels, target_labels, task="binary").item(),
                     ...
                     'auc': auroc(pred_labels, target_labels,
                                  task="binary").item(),
                     'prec': precision(pred_labels, target_labels, task="binary").item(),
                     'rec': recall(pred_labels, target_labels, task="binary").item(),
                     'spec': specificity(pred_labels, target_labels, task="binary").item(),
...
def add_epoch_perf(target_labels, pred_labels, loss, history):
    """
    Evaluate the various metrics for the supplied predicted and labelled targets, and append
